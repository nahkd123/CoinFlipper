...
public class CommandsManager implements CommandExecutor {
	...
	public CommandsManager() {
		...
		modules.add(new CommandAnimation(new String[] { Message.CMD_ANIMATION.getMessage(), "animation", "anim" },
				"animation", 0, -1, true));
	...
	public boolean onCommand(CommandSender sender, Command cmd, String cfL, String[] args) {
		...
		if (argsNum > 0) {
			for (CommandModule m : modules) {
				...
				// Debug.print(m.getLabels().toString() + ":" + m.getMinArgs() + ":" +
				// m.getMaxArgs() + ":" + args[0].toLowerCase());
				...
				if (!manageCommand(m, sender, argsNum - 1, senderPlayer))
					return true;
			...
			try {
				// Check argument before passing through manageCommand
				// If any exception thrown, this will print out help
				Double.parseDouble(args[0]);
				if (manageCommand(creationModule, sender, argsNum, senderPlayer)) creationModule.runCommand(sender, args);
				return true;
			} catch (Exception e) {
				sender.sendMessage(help);
			}
