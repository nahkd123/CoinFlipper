...
public class HistoryManager {
	...
	public Inventory getHistory(Player p) {
		PagedInventory gonnaBeHistory = new PagedInventory(
				Message.HISTORY_INVENTORY.getMessage().replaceAll("%player%", p.getName()),
				...
				"coinflipper_history_" + p.getName(), null, true);
			...
			public void run() {
				try {
					...
					while (rs.next()) {
						...
						ItemUtils.addToLore(historyId, Message.HISTORY_TIME.getMessage().replace("%time%", DateFormat.getDateTimeInstance().format(timeDate)));
						...
						String moneyStatus = won
								? Message.HISTORY_MONEY_WON.getMessage().replace("%money%", moneyReplace + "")
