...
public class SQLManager {
	...
	public void setup() {
		...
		if (!enabled) {
			...
			sql = new MySQL(conf.getString("server"), conf.getString("database"), conf.getString("user"),
		...
		if (!sql.tableExists("coinflipper_history")) {
			try {
				...
				PreparedStatement createGamesTable = conn.prepareStatement("CREATE TABLE coinflipper_history("
						+ "id INTEGER PRIMARY KEY " + autoIncr + "," + "time " + dateLong + " NOT NULL,"
						+ "winner CHAR(36)," + "loser CHAR(36) NOT NULL," + "moneyWon DOUBLE NOT NULL,"
						+ "moneyPot DOUBLE NOT NULL," + "tax DOUBLE NOT NULL" + ")");
	...
	private void convertOld(boolean sqlite) {
		...
		try {
			...
			if (sqlite) {
				...
				System.out.println(
						"[CoinFlipper] Conversion complete. Old stats table still available as coinflipper_stats_old");
			}
